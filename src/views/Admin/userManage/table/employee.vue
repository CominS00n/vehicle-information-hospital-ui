<template>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(user, i) in paginatedCars" :key="i" :class="`${i % 2 !== 0? 'bg-gray-100' : ''} hover:bg-slate-100 hover:shadow-md`">
              ...
              <td>{{ user.email }}</td>
              <td>
                <button @click="deleteUserEmployee(user.id)" class="hover:bg-slate-300 p-2 rounded-full">
  ...
  const { userDetails, getUserDetails, deleteUserDetail } = useEmployee()
  ...
  async function deleteUserEmployee(id) {
    Swal.fire({
  ...
  const filteredCars = computed(() => {
    ...
    return userDetails.value.filter((user) => {
      return (
